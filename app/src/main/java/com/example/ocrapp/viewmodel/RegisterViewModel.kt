    ...
    fun register(pass: String, view: View) {
        ...
        myUser?.let { theUser ->
            auth.createUserWithEmailAndPassword(theUser.email, pass).addOnSuccessListener {
                ...
                firestore.collection("users").document(theUser.uid.toString()).set(theUser)
                    .addOnSuccessListener {

                        if (theUser.type == "customer") {
                            Navigation.findNavController(view)
                                .navigate(R.id.action_registerFragment_to_customerActivity)
                        } else {
                            Navigation.findNavController(view)
                                .navigate(R.id.action_registerFragment_to_enterpriseActivity)
                        }
                    }
